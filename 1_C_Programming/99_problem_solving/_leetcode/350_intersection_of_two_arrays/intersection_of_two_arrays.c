/**
 * @file intersection_of_two_arrays.c
 * @author your name (you@domain.com)
 * 
 * @brief Given two integer arrays nums1 and nums2, return an array of their intersection. Each element in the result 
...
//structure of our data node, we use only singly linked list
typedef struct s_data_node
{
    int key;
    int data;
    // struct s_data_node *next; // no need as we are doing open addressing method
}s_data_node;
...
void insertNode(s_data_node **hash_table, int key)
{
    ...
    hash_table[hash_index]->data = key;

}

void deleteNode(s_data_node **hash_table, int key)
{
    int hash_func_idx = 0;
    int hash_index;
...
void freeTable(s_data_node **hash_table)
{
    int hash_func_idx = 0;
    int hash_index;
    static int pwe = 0;
    ...
    for( hash_func_idx = 0 ; hash_func_idx < TABLE_SIZE ; hash_func_idx++)
    {
        if((hash_table[hash_func_idx]) != NULL)
        {
            free(hash_table[hash_func_idx]);
        }
    }
...
int* intersect(int* nums1, int nums1Size, int* nums2, int nums2Size, int* returnSize)
{
    ...
    int idx;
    intersection_array = (int*) malloc(sizeof(int)*nums1Size);
    ...
    *returnSize = 0;
    ...
    {
        insertNode(hash_table_1, nums1[idx]);
    }
    
    for( idx = 0; idx < nums2Size ; idx++ )
    {
        if(searchTable(hash_table_1, nums2[idx]) == true)
        {
            deleteNode(hash_table_1, nums2[idx]);
            intersection_array[(*returnSize)++] = nums2[idx];
        }
    }

    freeTable(hash_table_1);
...
int main(int argc, char* argv[])
{
    ...
    printf("ARRAY:\n");
    printArray(result_array, returnSize);
