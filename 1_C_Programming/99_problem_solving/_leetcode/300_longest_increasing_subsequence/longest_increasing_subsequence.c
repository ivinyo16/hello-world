...
int lengthOfLISAux(int *nums, int numsSize,  int index, int prev)
{
    int ans = 0;
    int dont_take = 0;
    int take = 0;
    ...
    dont_take = lengthOfLISAux(nums, numsSize, index + 1, prev);
    if(nums[index] > prev)
    {
        take = 1 + lengthOfLISAux(nums, numsSize, index + 1, nums[index]);
    }
    ans = take >= dont_take ? take : dont_take;
...
{
    return lengthOfLISAux(nums, numsSize, 0, INT_MIN);
}
