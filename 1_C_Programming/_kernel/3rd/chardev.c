...
int init_module(void)
{
	...
	printk(KERN_INFO "the driver, create new file with \n");
...
/*
 * Called when a process closes a device file
 */
...
static int device_release(struct inode *inode, struct file *file)
{
	...
	/* 
 	 * Decrement the usage count, or else once you've opened the file
	 * the module will never be ridden.
	 */
...
/*
 * Called when a process, which already opened the dev file, attempt to
 * read it.
 */
			   ...
			   loff_t * offset)
{
	...
	while (length && *msg_Ptr)
	{
		 ...
		 * segment so "*" assignement won't work. We have to use
